...
void peakComparisonUnfolding()
{
	...
	TString saveName = "data/peakComparison.root";
	...
	for(int i=0;i<=nBinsTrue;i++){
		content = hTrue->GetBinContent(i+binShiftTrue);
		error = hTrue->GetBinError(i+binShiftTrue);
		hTrueRandom->SetBinContent(i,content);
		hTrueRandom->SetBinError(i,error);
	}
	...
	for(int i=0;i<=nBinsTrueDY;i++){
		content = hTrueDY->GetBinContent(i+binShiftTrueDY);
		error = hTrueDY->GetBinError(i+binShiftTrueDY);
		hTrueRandomDY->SetBinContent(i,content);
		hTrueRandomDY->SetBinError(i,error);
	}
	...
	c3->SaveAs("plots/unfoldedResults.png");
	c4->SaveAs("plots/unfoldedResultsDY.png");
